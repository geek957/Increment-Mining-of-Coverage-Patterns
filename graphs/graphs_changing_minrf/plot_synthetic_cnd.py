...
def plot1():
	y_plt = []
	lines1 = open("synthetic_cmine.csv",'r').readlines()
	lines2 = open("synthetic_increment.csv",'r').readlines()
	cmine,increment = get_values(lines1,lines2,1,7,1)
	# print ratios
	y_plt.append(cmine)
	y_plt.append(increment)
	x_plt = np.arange(450,601,25)
	X_plt = np.arange(425,601,25)
	x_ticks = []
	for i in X_plt:
		if(i%50==0):
			x_ticks.append(i)
	...
	title = "DB=50K rows,db=25K rows, minCS=0.3, maxOR=0.2"
	# title = "minrf = "+str(minRF)+" minCS = 0.5"
	# x_label = "minRF (DB = 50K, db = 25K)"
	x_label = "minRF ("+r'$ \times 10^{-4}$'+")"
	y_label = "NC ("+r'$ \times 10^3$'+")"
	legends = ('CMine','IncCMine')
	filename = "../images_final/synthetic_cnd_minRF.eps"
	plot_graph1(x_plt, y_plt, X_plt, x_ticks, y_ticks, title, x_label, y_label, legends, [0.0,25], [445,600], [0,7], filename, 1)
