...
def plot1():
	y_plt = []
	lines1 = open("bmspos_cmine.csv",'r').readlines()
	lines2 = open("bmspos_increment.csv",'r').readlines()
	cmine,increment = get_values(lines1,lines2,1,7,1)
	# print ratios
	y_plt.append(cmine)
	y_plt.append(increment)
	x_plt = np.arange(65,100,5)
	X_plt = np.arange(60,100,5)
	x_ticks = []
	for i in X_plt:
		if((i-5)%10==0):
			x_ticks.append(i)
	...
	title = "DB=100K rows,db=25K rows, minCS=0.5, maxOR=0.6"
	...
	filename = "../images_final/bmspos_cnd_minRF.eps"
	plot_graph1(x_plt, y_plt, X_plt, x_ticks, y_ticks, title, x_label, y_label, legends, [0.0,5], [63,95], [0,11], filename, 1)
